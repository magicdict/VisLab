...
export class Bar_BasicComponent implements OnInit {
  title = '柱状图-基本';
  Sample: OptionBase
  RainbowSample: OptionBase;
  RainbowSample_Dark: OptionBase;
  GradientSample: OptionBase;
  Sample_dark_GradientSample: OptionBase;
  RainbowSample_PictorialBar: OptionBase;
  ngOnInit(): void {

    let category = ['唐三', '戴沐白', "马红俊", "奥斯卡", "小舞", "宁荣荣", "朱竹清"];
    let value = [50, 100, 150, 70, 80, 120, 90];

    this.Sample = Bar.BarUtility.CreateBar(category, value);
    this.Sample.xAxis[0].axisLabel = { interval: 0, rotate: 45 }

    this.RainbowSample = Bar.BarUtility.CreateBar(category, value);
    ...
    imggrid.top = 10;
    imggrid.right = 10;
    let g = Graphic.CreateGraphic_Image("assets/image/小舞/头像.png", imggrid, null)
    this.RainbowSample.graphic = [g];

    this.RainbowSample_Dark = Bar.BarUtility.CreateBar(category, value);
    ...
    this.GradientSample = Bar.BarUtility.CreateBar(category, value);
    ...
    this.Sample_dark_GradientSample = Bar.BarUtility.CreateBar(category, value);
    ...
    this.RainbowSample_PictorialBar = Bar.BarUtility.CreatePictorialBar(category, value, false);
